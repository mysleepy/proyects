...
namespace MySleepy
{
    public partial class AddProveedor : Form
    {
        ...
        public static AddProveedor Instance(ConnectDB c,DataSet ds)
        {
            if (instance == null || instance.IsDisposed)
            {
                instance = new AddProveedor(c,ds);
            }
            return instance;
        }
        public static AddProveedor Instance(ConnectDB c,int id,DataSet ds)
        {
            if (instance == null || instance.IsDisposed)
            {
                instance = new AddProveedor(c,id,ds);
            }
            return instance;
        }
        public AddProveedor(ConnectDB conexion,DataSet ds)
        {
        ...
        public AddProveedor(ConnectDB conexion,int id,DataSet ds)
        {
            toolTip1 = new ToolTip();
            InitializeComponent();
            txtCIF.Enabled = false;
        ...
        {
            int codigo = Convert.ToInt32(e.KeyChar);
            if ((Char.IsDigit(e.KeyChar) || char.IsSymbol(e.KeyChar) || e.KeyChar.Equals('\'') || txtDNI.Text.Length >= 20) && (codigo != 8))
            {
                e.Handled = true;
            }
            else
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            if (compruebaCampos() == false)
            {
                ...
                {
                    DataView dv = new DataView(ds.Tables["PROVEEDOR"]);
                    dv.Sort="IDPROVEEDOR";
