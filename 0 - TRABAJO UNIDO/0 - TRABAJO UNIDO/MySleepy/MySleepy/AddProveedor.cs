...
namespace MySleepy
{
    public partial class AddProveedor : Form
    {
        ...
        public static AddProveedor Instance(ConnectDB c, DataSet ds, Proveedor daddy)
        {
            if (instance == null || instance.IsDisposed)
            {
                instance = new AddProveedor(c, ds, daddy);
            }
            return instance;
        }
        public static AddProveedor Instance(ConnectDB c, int id, DataSet ds, Proveedor daddy)
        {
            if (instance == null || instance.IsDisposed)
            {
                instance = new AddProveedor(c, id, ds, daddy);
            }
            return instance;
        }
        public AddProveedor(ConnectDB conexion, DataSet ds, Proveedor daddy)
        {
            this.daddy = daddy;
        ...
        public AddProveedor(ConnectDB conexion, int id, DataSet ds, Proveedor daddy)
        {
            this.daddy = daddy;
            toolTip1 = new ToolTip();
            InitializeComponent();
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            if (compruebaCampos() == false)
            {
                ...
                if (mod == false)
                {
                    Object resultadoSql = conexion.DLookUp("IDPROVEEDOR", "PROVEEDORES", "IDPROVEEDOR =1");
                    int id;
                    if (Convert.ToInt32(resultadoSql) == -1) { id = 1; }
