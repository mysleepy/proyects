...
namespace MySleepy
{
    public partial class AddNuevoArticulo : Form
    {
            ...
            if (numero == 0)
            {
                if (comprobarReferencia(Convert.ToInt32(txtReferencia.Text)))
                {
                    ...
                    sentencia = "INSERT INTO ARTICULOS(IDARTICULO,REFCOMPOSICION,REFMEDIDA,PRECIO,ELIMINADO,NOMBRE,REFERENCIA,STOCK)" +
                                                " VALUES(" + nuevoid + ",'" + cboComposicion.SelectedIndex + "','" + cboMedida.SelectedIndex + "'," + txtPrecio.Text + ",0,'" + nombreArticulo.ToUpper() + "'," + Convert.ToInt32(txtReferencia.Text) + ","+Convert.ToInt32(nStock.Value)+")";
                ...
                {
                    MessageBox.Show("Error en la referencia de producto, ya existe");
        ...
        {
            String select = "SELECT REFERENCIA FROM ARTICULOS WHERE REFERENCIA=" + referencia;
